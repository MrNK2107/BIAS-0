...
export default function HiddenBiasExplorer({
  subgroups = [],
}: HiddenBiasExplorerProps) {
  ...
  const filteredSubgroups = useMemo(() => {
    ...
    if (selectedAttributes.length > 0) {
      filtered = filtered.filter((sg) =>
        selectedAttributes.some((attr) =>
          Object.keys(sg.attributes).includes(attr),
        ),
      );
    }
    ...
    return filtered.sort(
      (a, b) => Math.abs(b.metricValue) - Math.abs(a.metricValue),
    );
  }, [subgroups, selectedAttributes]);

  const worstSubgroup =
    filteredSubgroups.length > 0 ? filteredSubgroups[0] : null;
  ...
  return (
    <div className="card" style={{ marginBottom: "24px" }}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <p className="helper">
          Group-level fairness metrics can hide intersectional bias. This tool
          surfaces specific subgroups where the model performs significantly
          worse.
        </p>
      </div>
        ...
        >
          ...
          <div>
            ...
            <div style={{ fontSize: "1.1rem", color: "var(--text-primary)" }}>
              <strong>{worstSubgroup.definition}</strong> →{" "}
              {Math.abs(worstSubgroup.metricValue * 100).toFixed(0)}% lower{" "}
              {worstSubgroup.metricName.toLowerCase()}
            </div>
            <div
              style={{
                fontSize: "0.875rem",
            ...
            {availableAttributes.map((attr) => (
              <button
                ...
                style={{
                  ...
                  backgroundColor: selectedAttributes.includes(attr)
                    ? "rgba(200, 157, 124,0.12)"
                    : "rgba(255,255,255,0.02)",
                  color: selectedAttributes.includes(attr)
                    ? "var(--accent)"
                    : "var(--text-secondary)",
                  fontSize: "0.85rem",
                  cursor: "pointer",
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: "12px 8px" }}>
                          <span
                            className={`pill ${isNegative ? "red" : "green"}`}
