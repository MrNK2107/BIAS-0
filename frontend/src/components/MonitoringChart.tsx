...
const CustomDot = (props: CustomDotProps) => {
  ...
  return (
    <g onClick={() => onDotClick?.(payload)} style={{ cursor: 'pointer' }}>
      <circle cx={cx} cy={cy} r={10} fill="rgba(162, 74, 70,0.22)" stroke="none" />
      <circle cx={cx} cy={cy} r={4} fill="#A24A46" stroke="#F1F1F1" strokeWidth={1.5} />
    </g>
  );
};
...
export default function MonitoringChart({ events, viewMode, incidents = [], onDotClick }: MonitoringChartProps) {
  ...
  const colors = ['#C89D7C', '#A24A46', '#E8C6A0', '#A56C40', '#9F3E40', '#F1F1F1', '#CDA47A', '#7F848A'];
        ...
        <ComposedChart data={viewMode === 'overall' ? events : transformedData}>
          <defs>
            <linearGradient id="monitorGradient" x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stopColor="#C89D7C" stopOpacity={0.35} />
              <stop offset="100%" stopColor="#C89D7C" stopOpacity={0.05} />
            </linearGradient>
            <linearGradient id="alertZoneGradient" x1="0" x2="0" y1="0" y2="1">
              <stop offset="0%" stopColor="#A24A46" stopOpacity={0.14} />
              <stop offset="100%" stopColor="#A24A46" stopOpacity={0.03} />
            </linearGradient>
          </defs>
          ...
          {viewMode === 'overall' && alertZones.map((zone, i) => (
            <ReferenceArea key={`zone-${i}`} x1={zone.start} x2={zone.end} fill="rgba(162, 74, 70,0.1)" stroke="none" />
          ))}

          {/* Incident reference lines */}
          {viewMode === 'overall' && incidents.filter(m => m.type === 'incident').map((m, i) => (
            <ReferenceLine key={`inc-${i}`} x={m.timestamp} stroke="#A24A46" strokeDasharray="6 4" strokeWidth={1.5} label={{ value: '⚠', position: 'top', fill: '#A24A46', fontSize: 14 }} />
          ))}

          {/* Drift reference lines */}
          {viewMode === 'overall' && incidents.filter(m => m.type === 'drift').map((m, i) => (
            <ReferenceLine key={`drift-${i}`} x={m.timestamp} stroke="#A24A46" strokeDasharray="4 4" strokeWidth={1.5} label={{ value: '↕', position: 'top', fill: '#A24A46', fontSize: 14 }} />
          ))}

          {/* Threshold line */}
          {viewMode === 'overall' && (
            <ReferenceLine y={70} stroke="rgba(200, 157, 124,0.45)" strokeDasharray="8 6" label={{ value: 'Fair threshold', position: 'right', fill: '#C89D7C', fontSize: 11 }} />
          )}
          ...
          {viewMode === 'overall' ? (
            <>
              ...
              <Line
                ...
                activeDot={{ r: 7, fill: '#C89D7C', stroke: '#F1F1F1', strokeWidth: 2 }}
              />
            </>
          ) : (
