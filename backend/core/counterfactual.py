...
def run_counterfactual_test(
    ...
    baseline_gaps = (
        fairness_gaps(
            y_pred, prepared.y_test, df.loc[prepared.y_test.index, sensitive_col]
        )
        if sensitive_col in df.columns
        else dict(ZERO_GAPS)
    )
    baseline_score = fairness_score_from_gaps(
        baseline_gaps, metric_weights=metric_weights
    )

    flip_breakdown: dict[str, dict[str, float]] = {}
    sensitive_values = (
        list(df[sensitive_col].dropna().astype(str).unique())
        if sensitive_col in df.columns
        else []
    )
    total_flips = 0
    ...
    for original_value in sensitive_values:
        for flipped_value in sensitive_values:
            ...
            subset = (
                prepared.X_test[
                    prepared.X_test[sensitive_col].astype(str) == original_value
                ].copy()
                ...
                sample_flips.append(
                    {
                        ...
                        "original_decision": (
                            "approved"
                            if int(original_pred[idx_in_subset]) == 1
                            else "rejected"
                        ),
                        "flipped_decision": (
                            "approved"
                            if int(flipped_pred[idx_in_subset]) == 1
                            else "rejected"
                        ),
                    }
                )
