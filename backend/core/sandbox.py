...
def _apply_fix(
    df: pd.DataFrame, fix: dict[str, Any], sensitive_cols: list[str], target_col: str
    ...
    if fix_type == "feature_level" and "remove" in description:
        ...
        if fix_id.startswith("remove_"):
            ...
            feature = next(
                (
                    ...
                    == feature_pattern.lower().replace(" ", "_")
                ),
                None,
            )
        ...
        target = target_col if target_col in modified.columns else modified.columns[-1]
        ...
        mitigator.fit(
            prepared.X_train, prepared.y_train, sensitive_features=sensitive_feature
        )
...
def run_sandbox_simulation(
    ...
    def score_frame(
        frame: pd.DataFrame, name: str, notes: str, model_override: Any = None
    ) -> dict[str, Any]:
        prepared = prepare_split(frame, target_col)
        if model_override:
            y_pred = pd.Series(
                model_override.predict(prepared.X_test), index=prepared.y_test.index
            )
        else:
            model = build_and_fit_default(prepared)
            y_pred = pd.Series(
                model.predict(prepared.X_test), index=prepared.y_test.index
            )
        ...
        gaps = (
            fairness_gaps(
                y_pred,
                prepared.y_test,
                frame.loc[prepared.y_test.index, sensitive_cols[0]],
            )
            if sensitive_cols
            else dict(ZERO_GAPS)
        )
    ...
    for fix in fixes_to_apply:
        ...
        scenarios.append(
            score_frame(
                modified,
                fix.get("description", fix.get("fix_id", "Scenario")),
                fix.get("estimated_impact", "Simulated fix"),
    ...
    if any(f.get("fix_id") == "threshold_tune" for f in fixes_to_apply):
        ...
        model = build_and_fit_default(prepared)
        y_pred_tuned, _ = _apply_threshold_tuning(
            df, sensitive_cols, target_col, prepared, model
        )
    ...
    recommendation = (
        f"{best['name']} offers the best fairness score among the simulated options."
    )
