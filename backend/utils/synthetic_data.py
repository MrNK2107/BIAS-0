...
def _random_choice(
    rng: np.random.Generator, values: list[str], probabilities: list[float], size: int
) -> np.ndarray:
...
def generate_loan_dataset(rows: int = 5000, seed: int = 42) -> pd.DataFrame:
    ...
    gender = _random_choice(
        rng, ["male", "female", "non-binary"], [0.52, 0.42, 0.06], rows
    )
    caste = _random_choice(
        rng, ["general", "obc", "sc", "st"], [0.42, 0.32, 0.16, 0.10], rows
    )
    age = rng.integers(21, 65, rows)
    education = _random_choice(
        rng, ["graduate", "postgraduate", "high_school"], [0.45, 0.25, 0.30], rows
    )
    zip_prefix_map = {
        "general": ["100", "101", "102"],
        "obc": ["200", "201", "202"],
    ...
    income = income + np.where(
        gender == "male", 16000, np.where(gender == "female", -12000, -2500)
    )
    income = income + np.where(
        caste == "general",
        7000,
        np.where(caste == "obc", 3000, np.where(caste == "sc", -4500, -6000)),
    )
    ...
    score = (
        ...
        + np.where(
            ...
            0.22,
            np.where(education == "graduate", 0.12, -0.1),
        )
        + np.where(gender == "male", 0.9, np.where(gender == "female", -0.5, -0.1))
        + np.where(
            caste == "general",
            0.75,
            np.where(caste == "obc", 0.2, np.where(caste == "sc", -0.5, -0.6)),
        )
        - 0.03 * (age < 24)
    )
    ...
    target[general_mask] = np.maximum(
        target[general_mask], (rng.random(general_mask.sum()) < 0.75).astype(int)
    )
...
def generate_hiring_dataset(rows: int = 3000, seed: int = 7) -> pd.DataFrame:
    rng = np.random.default_rng(seed)
    gender = _random_choice(
        rng, ["male", "female", "non-binary"], [0.50, 0.43, 0.07], rows
    )
    ethnicity = _random_choice(
        rng, ["group_a", "group_b", "group_c"], [0.6, 0.25, 0.15], rows
    )
    age = rng.integers(22, 55, rows)
    years_experience = np.clip(rng.normal(6, 3, rows), 0, 20)
    university_tier = np.where(
        gender == "male",
        rng.choice([1, 2, 3], size=rows, p=[0.5, 0.35, 0.15]),
        rng.choice([1, 2, 3], size=rows, p=[0.25, 0.45, 0.30]),
    )
    skills_score = np.clip(rng.normal(70, 12, rows), 20, 100)
    interview_score = np.clip(rng.normal(68, 14, rows), 15, 100)
    score = (
        0.018 * skills_score
        + 0.015 * interview_score
        + 0.28 * years_experience
        + np.where(
            university_tier == 1, 0.25, np.where(university_tier == 2, 0.08, -0.08)
        )
    )
    score = score + np.where(
        gender == "male", 0.6, np.where(gender == "female", -0.4, -0.15)
    )
